class ComponentAbstract {
  constructor() {
    if (this.constructor == ComponentAbstract) {
  ...
  render(dataCars, resultContainer) {
    resultContainer.innerHTML = ""
    try {
      if (dataCars.length) {
        dataCars.map((data) => {
          const fiUser = `<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
          ...
          const fiSettings = `<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          >`;
          const fiCalendar = `<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
          ...
          >`;
          const newContentResult = document.createElement("div");
          newContentResult.setAttribute("class", "contentResult");

          const newContentDetail = document.createElement("div");
          newContentDetail.setAttribute("class", "contentDetail");

          const newImage = document.createElement("img");
          newImage.setAttribute("alt", "car image");
          newImage.setAttribute("src", data.image);

          const newDiv = document.createElement("div");
          newDiv.innerHTML += `<p>${data.type}</p>`;
          newDiv.innerHTML += `<h3>RP ${data.rentPerDay.toLocaleString("id-ID")} / hari</h3>`;
          newDiv.innerHTML += `<p>${data.description}</p>`;
          newDiv.innerHTML += `<p>${fiUser} <span style="margin-left: 8px">${data.capacity} orang</span></p>`;
          newDiv.innerHTML += `<p>${fiSettings} <span style="margin-left: 8px">${data.transmission}</span></p>`;
          newDiv.innerHTML += `<p>${fiCalendar} <span style="margin-left: 8px"> Tahun ${data.year} </span></p>`;

          const btnChoose = document.createElement("button");
          btnChoose.classList.add("btn", "btn-success", "btn-lg");
          btnChoose.style.width = "100%";
          btnChoose.innerText = "Pilih Mobil";

          newContentDetail.appendChild(newImage);
          newContentDetail.appendChild(newDiv);

          newContentResult.appendChild(newContentDetail);
          newContentResult.appendChild(btnChoose);

          resultContainer.appendChild(newContentResult);
        });
      } else {
        let notFoundComp = document.createElement("h3")
        notFoundComp.textContent = 'Data tidak ditemukan'
        resultContainer.appendChild(notFoundComp)
      }
...
class Car extends ComponentAbstract {
  constructor() {
    super();
  }
}
